...
fe_in = supergaussian_e(ve, 300, 0, 3)
fe_in1 = supergaussian_e(ve, 300, 100, 3)
fH_in = supergaussian_H(vH, 200, 1e5, 3)

plt.scatter(ve, fe_in, color = "orange")
plt.plot(ve, fe_in1)
# plt.plot(vH, fH_in)
...
# epw_Pw_in = epw_Pw_in.value
# iaw_Pw_in = iaw_Pw_in.value
...
efn_TORCH = supergaussian_e_TORCH(e_velocity_axes_TORCH, torch.exp(ln_e_T), e_vd, e_p) # torch.tensor(fe_in_TORCH, dtype=torch.float64)
ifn_TORCH = supergaussian_H_TORCH(i_velocity_axes_TORCH, torch.exp(ln_i_T), i_vd, i_p) # torch.tensor(fH_in_TORCH, dtype=torch.float64)
...
# Call optimizer
opt = torch.optim.NAdam((ln_e_T, e_vd, e_p, ln_i_T, i_vd, i_p, ln_n),lr=1e-3, betas=[0.9985, 0.999])
...
for i in trange(4000):    # Can reduce to 3000
    opt.zero_grad()
    ...
    # Make hist lists for parameters to track grads
    # print("gradients:", n_TORCH.grad.data) #, ln_e_T.grad.data)  # efn_TORCH.grad.data, ifn_TORCH.grad.data)
...
plt.plot(ve, efn_TORCH.detach().numpy(), label = "NAdam VDF", color="orange")
...
e_p_fit = e_p_optim.detach()
n_fit = n_optim.detach()

opt_iaw = torch.optim.NAdam((ln_i_T, i_vd, i_p), lr=1e-3, betas=[0.9985, 0.999])
...
efn_TORCH_iaw = supergaussian_e_TORCH(e_velocity_axes_TORCH, torch.exp(ln_e_T), e_vd_fit, e_p_fit)
ifn_TORCH_iaw = supergaussian_H_TORCH(i_velocity_axes_TORCH, torch.exp(ln_i_T), i_vd, i_p)
...
for i in trange(4000):    # Can reduce to 2000
    opt_iaw.zero_grad()

    # Reconstruct efn and ifn using updated Ts, vds, ps
    efn_TORCH_iaw = supergaussian_e_TORCH(e_velocity_axes_TORCH, torch.exp(ln_e_T), e_vd_fit, e_p_fit)
    ifn_TORCH_iaw = supergaussian_H_TORCH(i_velocity_axes_TORCH, torch.exp(ln_i_T), i_vd, i_p)
...
ifn_final = supergaussian_H_TORCH(i_velocity_axes_TORCH, torch.exp(ln_i_T), i_vd, i_p)
