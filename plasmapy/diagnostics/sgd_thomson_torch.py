...
def fast_spectral_density_arbdist(
    ...
    longArgI = (i_velocity_axes - ion_vel_1d) / (torch.sqrt(torch.tensor(2)) * vTi)
    longArgI = torch.flatten(longArgI)
    ...
    for i, j in (notches):
        ...
        x0 = torch.argmin(torch.abs(wavelengths - notches[i]))
        x1 = torch.argmin(torch.abs(wavelengths - notches[j]))
        bools[x0:x1] = False
