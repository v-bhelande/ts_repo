...
# DERIVATIVE FUNCTION CHANGED BEGINS HERE
...
def derivative(f, x, order):
    ...
    if order == 1:
        ...
        return F.conv1d(f,kernel1,padding='same')
    elif order == 2:
        kernel2 = (pt.tensor([[[-1./12.,4./3.,-5./2.,4./3.,-1./12.]]], dtype=pt.float64))/(dx**2)     # Start to lose degrees of precision for 2nd order, is that ok?
                                                                                    # Refer to test case 2 and EMAIL MARK ABOUT THIS TODAY!!!
        return F.conv1d(f,kernel2,padding='same')
    else:
        print("You can only choose an order of 1 or 2...")

# DERIVATIVE FUNCTION CHANGED ENDS HERE

# CHI FUNCTION CHANGED BEGINS HERE

# Ratchet interpolate function import for chi: https://github.com/pytorch/pytorch/issues/50334

def interpolate(x: pt.Tensor, xp: pt.Tensor, fp: pt.Tensor) -> pt.Tensor:
    ...
    fp = fp.reshape(-1)    # TODO: Temporary fix, change this later
    ...
    indicies = pt.sum(pt.ge(x[:, :, None], xp[:, None, :]), -1) - 1  #torch.ge:  x[i] >= xp[i] ? true: false
    indicies = pt.clamp(indicies, 0, m.shape[-1] - 1)

    line_idx = pt.linspace(0, indicies.shape[0], 1, device=indicies.device).to(pt.long)
...
# Helper function to reshape input tensor to 2D

def restore2D(list2D, tensaurus):
  # print("list2D:", list2D)
  tensaurus = pt.reshape(tensaurus, (list2D[0],list2D[1]))
  return tensaurus
...
def chiPT(
    ...
    g = interpolate(xi, u_axis, fPrime)
    g = pt.select(g,0,0)   # Slice into 1D array
    # print("g Interpolation:", g)

    gPrime = interpolate(xi, u_axis, fDoublePrime)
...
# Helper Function to convert result back into np array
